...
class HTTPServer(object):
    ...
    def client_handler(self, service_client_socket):
        ...
        if request_url.endswith(".html"):
            ...
            status, headers, response_body = shoujinzhe.app(env)
    ...
    @staticmethod
    def static_response(request_url, service_client_socket):
        """处理静态页面请求"""
        try:
            # url_list = request_url.split('/')
            # url = url_list[len(url_list)-1]
            # if url.endswith(".css"):
            #     file = open("../templates/css/" + url)
            # else:
            #     file = open("img/" + url)
            # print("请求路径为：../templates", request_url)
            print('*'*20)
            print(request_url)
            print('*' * 20)

            file = open("."+request_url, "rb")  # TODO 图片、音频等静态资源，必须用二进制文件进行读取
            response_body = file.read()
            print("当前路径为：.%s" % request_url)
            print(os.getcwd())
            ...
            # 打开404页面，读取内容，并替换指定的部分，拼合成相应体
            with open("../templates/404.html", encoding="utf-8") as f:  # TODO linux系统下可以不用写encoding
                content = f.read()
                # 内容替换
            import time
            content = re.sub(r'{%content%}', "我是templates文件夹下的静态页面！"+time.ctime(), content)

            response_body = content
            # response_body = response_body.encode()  # 转成二进制码


        # finally中的代码是一定会执行的代码
        # 一定需要执行的代码, 或者expect和else中都需要执行的代码

        # 响应行           响应头           空行      响应体
        response_data = response_line + response_headers + "\r\n"
        response_data = response_data.encode()  # TODO 先转换成二进制
        response_data = response_data + response_body  # TODO 再与读取的图片信息进行拼合
